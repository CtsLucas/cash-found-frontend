...
function InvoicesList({
  ...
  if (sortedInvoiceMonths.length === 0) {
    return <p className="p-4 text-center text-muted-foreground">{t('cards.no_invoices')}</p>;
  }
            ...
            {selectedTransactions.map((t) => (
              <TableRow key={t.id}>
                ...
                <TableCell className="text-right">
                  {formatCurrency(t.amount - (t.deduction || 0))}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
...
const CardsSkeleton = () => (
  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
    {Array.from({ length: 4 }).map((_, i) => (
      <Skeleton key={i} className="aspect-[85.6/53.98] w-full rounded-xl" />
    ))}
  </div>
);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center">
        <h1 className="font-headline text-3xl font-bold tracking-tight">{t('cards.title')}</h1>
            ...
            return (
              <div key={card.id} className="w-full max-w-[320px]">
                ...
                >
                  ...
                  <AddCardSheet
                    isOpen={isSheetOpen && editingCard?.id === card.id}
                  ...
                  <footer className="flex items-end justify-between text-xs">
                    <span>
                      {t('due')}: {card.dueDate ?? 'N/A'}
                    </span>
      ...
      {selectedCardId && (
        <UICard>
          <CardHeader>
            <CardTitle>
              {t('cards.invoices_for')} {cards?.find((c) => c.id === selectedCardId)?.cardName}
            </CardTitle>
          ...
          <CardContent>
            {isLoadingTransactions ? (
              ...
              <InvoicesList cardId={selectedCardId} transactions={allTransactions} />
            )}
          </CardContent>
        </UICard>
      )}
