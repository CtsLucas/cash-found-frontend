...
import { CreditCard, LayoutDashboard, ReceiptText, Settings, Shapes } from 'lucide-react';
...
export function MainNav({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  return (
    <nav className="grid items-start gap-2 px-2 text-sm font-medium sm:py-5 lg:px-4">
      {navItems.map((item) => {
        const isActive =
